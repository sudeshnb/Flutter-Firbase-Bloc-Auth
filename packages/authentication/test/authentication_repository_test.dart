...
import 'package:auth/src/core/cache/cache.dart';
...
void main() {
  ...
  group('AuthenticationRepository', () {
    ...
    late AuthRepo authenticationRepository;
    ...
    test('creates FirebaseAuth instance internally when not injected', () {
      expect(AuthRepo.new, isNot(throwsException));
    });
    ...
    group('currentUser', () {
      test('returns User.empty when cached user is null', () {
        when(
          () => cache.read(key: AuthRepo.userCacheKey),
        ).thenReturn(null);
        expect(
      ...
      test('returns User when cached user is not null', () async {
        when(
          () => cache.read<User>(key: AuthRepo.userCacheKey),
        ).thenReturn(user);
